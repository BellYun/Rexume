#!/usr/bin/env node
/**
 * TBT 비교: pdfOptimized (baseline) vs pdfOpt15-Worker (OffscreenCanvas+Worker)
 *
 * 사용:
 *   node bench/tbt-opt15-worker.js
 *
 * 환경변수:
 *   RUNS=5           반복 횟수 (기본 5)
...
const TEST_URLS = [
  {
    ...
    name: 'Baseline (RAF+IO, 메인 스레드 렌더)',
    shortName: 'baseline',
  },
  {
    url: `${BASE_URL}/pdf-bench/opt15?url=${PDF_URL}`,
    name: 'Opt15 (Worker+OffscreenCanvas)',
    shortName: 'opt15-worker',
  },
];
...
function printTable(summaries) {
  ...
  console.log(`\n${SEP}`);
  console.log(` TBT 비교: Baseline (RAF+IO)  vs  Opt15 (Worker+OffscreenCanvas)  [CPU ${CPU_THROTTLE}x · ${RUNS}회]`);
  console.log(`${SEP}`);
  console.log(
    `${'버전'.padEnd(34)} ${'TBT median'.padStart(11)} ${'vs baseline'.padStart(12)} ${'TBT avg'.padStart(9)} ${'FCP'.padStart(7)} ${'LongTask'.padStart(9)} ${'maxDur'.padStart(7)}`
  );
  console.log(`${'─'.repeat(34)} ${'─'.repeat(11)} ${'─'.repeat(12)} ${'─'.repeat(9)} ${'─'.repeat(7)} ${'─'.repeat(9)} ${'─'.repeat(7)}`);
  ...
  for (const s of summaries) {
    ...
    console.log(
      `${s.version.padEnd(34)} ${(s.tbt.median + 'ms').padStart(11)} ${sign.padStart(12)} ${(s.tbt.avg + 'ms').padStart(9)} ${(s.fcp.median + 'ms').padStart(7)} ${String(s.longTaskCount.avg).padStart(9)} ${(s.longTaskMax.median + 'ms').padStart(7)}`
    );
  }
  console.log(`${SEP}`);
...
async function main() {
  console.log(`\n${'═'.repeat(60)}`);
  console.log(` TBT 벤치마크: Baseline vs pdfOpt15-Worker`);
  ...
  const outFile = path.join(outDir, `tbt-opt15-worker-${ts}.json`);
  fs.writeFileSync(
    outFile,
