...
const RUNS_PER_URL = 3; // 3회로 줄임
const HEADLESS = true;

// Docker 환경에서는 nextjs:3000, 일반 환경에서는 localhost:3000
...
const TEST_URLS = [
  ...
  {
    url: `${BASE_URL}/feedback/4?version=opt1-raf`,
    name: 'Opt1: RAF 페인트 안정화',
    shortName: 'opt1-raf'
  },
  {
    url: `${BASE_URL}/feedback/4?version=opt2-callback-ref`,
    name: 'Opt2: Callback Ref 패턴',
    shortName: 'opt2-callback-ref'
  },
  {
    url: `${BASE_URL}/feedback/4?version=opt3-combined`,
    name: 'Opt3: Combined (RAF + Callback)',
    shortName: 'opt3-combined'
  }
];
...
(async () => {
  console.log('\n🚀 PDF 첫페이지 렌더링 성능 벤치마크 - 그룹1 (Opt 1,2,3 + 기준선)');
  ...
  for (const { url, name, shortName } of TEST_URLS) {
    ...
    // 각 URL 테스트 후 10초 대기 (브라우저 완전 종료 대기)
    console.log('\n💤 브라우저 정리 대기 (10초)...\n');
  ...
  const outputPath = path.join(outDir, `pdf-firstpage-group1-${timestamp}.json`);
  
  const summary = {
    timestamp: new Date().toISOString(),
    group: 'group1',
    description: '최적화 1,2,3 + 기준선',
    config: {
