#!/usr/bin/env node
/**
 * INP — completion RAF 배칭 효과 측정
 *
 * 비교 대상:
 * 1. Basic         — 없음
 * 2. Simple (IO)   — IntersectionObserver only
 * 3. IO+RAF(NoLimit) — initiation만 RAF
 * 4. IO+RAF+Batch  — initiation + completion 모두 RAF (NEW)
 * 5. IO+RAF+Limit  — initiation RAF + CONCURRENT_LIMIT=3
 */
...
function printTable(summaries) {
  ...
  console.log(`\n${SEP}`);
  console.log(` completion RAF 배칭 효과 — INP 측정  [CPU ${CPU_THROTTLE}x · ${RUNS}회 · 12초]`);
  console.log(`${SEP}`);
...
async function main() {
  console.log(`\n${'═'.repeat(70)}`);
  console.log(` completion RAF 배칭 효과 측정`);
  console.log(` CPU ${CPU_THROTTLE}x · ${RUNS}회 · Basic / IO / IO+RAF / IO+RAF+Batch / IO+RAF+Limit`);
  ...
  const outFile = path.join(outDir, `inp-raf-batch-${ts}.json`);
  fs.writeFileSync(outFile, JSON.stringify({ summaries, allResults, meta: {
    ...
    versions:'Basic / Simple(IO) / IO+RAF(NoLimit) / IO+RAF+Batch / IO+RAF+Limit=3',
  }}, null, 2));
  console.log(`\n결과 저장: ${outFile}`);
}
