...
import PDF from "../pdfOpt9-RAF-NoPaint-NoBatch/PDF";
...
const PDFViewer = ({
  ...
  const renderPageImmediately = useCallback((pageNumber: number) => {
    const el = pageElements.current.get(pageNumber);
    if (!el) return;
    if (el.rendered?.()) return;

    if (typeof el.renderPage === "function") {
      void el.renderPage();
    }
  }, []);

  // Shared IO 생성/정리 (배칭/스케줄러 없이 즉시 렌더)
  useEffect(() => {
  ...
  }, [pdf, numPages, renderPageImmediately]);
