...
const PDFViewer = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        console.log(`[Opt1-RAF] PDF 로딩 시작: ${pdfSrc}`);
        ...
        console.log('[Opt1-RAF] 워커 초기화 완료, PDF 문서 로딩 시작');
        ...
        console.log(`[Opt1-RAF] PDF 문서 로딩 완료: ${loaded.numPages}페이지`);
        ...
        if (cancelled) return;
        console.error("[Opt1-RAF] PDF 로딩 실패:", e);
      ...
      (entries) => {
        entries.forEach(async (entry) => {
          ...
          if (entry.isIntersecting) {
            console.log(`[Opt1-RAF] Page ${pageNumber} is intersecting`);
            
            // 이미 렌더링되었는지 확인
            if (pageElement.rendered && pageElement.rendered()) {
              console.log(`[Opt1-RAF] Page ${pageNumber} already rendered`);
              return;
            }

            // 직접 렌더링
            if (pageElement.renderPage) {
              console.log(`[Opt1-RAF] Rendering page ${pageNumber} directly`);
              await pageElement.renderPage();
            } else {
              console.log(`[Opt1-RAF] renderPage function not found for page ${pageNumber}, element:`, pageElement);
              // 잠시 후 다시 시도
              setTimeout(() => {
                if (pageElement.renderPage) {
                  console.log(`[Opt1-RAF] Retry rendering page ${pageNumber}`);
                  pageElement.renderPage();
                } else {
                  console.log(`[Opt1-RAF] Still no renderPage function for page ${pageNumber}`);
                }
              }, 100);
            }
          }
  ...
  if (!pdf) return <div>PDF 로딩 중... (Opt1-RAF)</div>;
