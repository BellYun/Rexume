...
const PDFViewer = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log(`[Opt7-AllScheduling] PDF 로딩 시작: ${pdfSrc}`);
        ...
        console.log(`[Opt7-AllScheduling] PDF 문서 로딩 완료: ${loaded.numPages}페이지`);
        setPdf(loaded);
        setNumPages(loaded.numPages);
      } catch (e: any) {
        if (!cancelled) {
          console.error("[Opt7-AllScheduling] PDF 로딩 실패:", e);
          setErr(e?.message ?? "PDF 로딩 실패");
        }
      } finally {
  ...
  const flushInRaf = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      // 🎯 최적화 6: 우선순위 = viewport 중심으로부터의 거리(px)
        ...
        .forEach(({ n, el, priority }) => {
          ...
          schedulerRef.current.enqueue({
            ...
            run: async () => {
              ...
              console.log(`[Opt7-AllScheduling] Rendering page ${n} (priority: ${priority.toFixed(0)})`);
              await el.renderPage();
            },
          });
        });
  ...
  if (loading || !pdf) return <div>PDF 로딩 중... (Opt7-AllScheduling)</div>;
