...
const PDFViewer = ({
  ...
  const flushInRaf = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      // 🎯 최적화 6: viewport 중심으로부터의 거리 기반 우선순위 정렬
      ...
      pages
        ...
        .sort((a, b) => a.priority - b.priority) // 우선순위 낮을수록 먼저
        .forEach(({ n, el, priority }) => {
          ...
          // 직접 렌더링 (스케줄러 없음)
          if (el.renderPage) {
            console.log(`[Opt6-Priority] Rendering page ${n} in rAF batch (priority: ${priority.toFixed(0)})`);
            el.renderPage();
          }
        });
    });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        console.log(`[Opt6-Priority] PDF 로딩 시작: ${pdfSrc}`);
        ...
        console.log('[Opt6-Priority] 워커 초기화 완료, PDF 문서 로딩 시작');
        ...
        console.log(`[Opt6-Priority] PDF 문서 로딩 완료: ${loaded.numPages}페이지`);
        ...
        if (cancelled) return;
        console.error("[Opt6-Priority] PDF 로딩 실패:", e);
  ...
  // IntersectionObserver - RAF 배칭 + 우선순위 정렬
  useEffect(() => {
      ...
      (entries) => {
        ...
        for (const entry of entries) {
          ...
          if (entry.isIntersecting) {
            ...
            console.log(`[Opt6-Priority] Page ${n} added to pending batch`);
          } else {
  ...
  if (!pdf) return <div>PDF 로딩 중... (Opt6-Priority)</div>;
