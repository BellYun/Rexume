...
const PDF = forwardRef<HTMLDivElement, PDFProps>(({
  ...
  const renderPage = useCallback(async () => {
    ...
    try {
      console.log(`[Opt3-Combined] Starting render for page ${pageNumber}`);
      ...
      // 🎯 최적화 1: requestAnimationFrame을 통한 페인트 안정화
      ...
      console.log(`[pdfOpt3-Combined] 페이지 ${pageNumber} 렌더링 완료: ${metrics.totalMs.toFixed(1)}ms (paint: ${metrics.paintMs.toFixed(1)}ms)`);
  ...
  // 🎯 최적화 2: useCallback을 사용한 ref 함수 메모이제이션
  const setHostRef = useCallback(
    (el: HTMLDivElement | null) => {
      ...
      console.log(`Page ${pageNumber}: renderPage function attached (Opt3-Combined)`);
