...
const PDFViewer = ({
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && (window as any).__reactPerformanceTracker) {
      const startTime = (window as any).__reactPerformanceTracker.renderStart('PDFViewerOpt3Combined');
      return () => (window as any).__reactPerformanceTracker.renderEnd('PDFViewerOpt3Combined', startTime);
    }
  });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && (window as any).setStateTracker) {
      (window as any).setStateTracker.trackStateChange('PDFViewerOpt3Combined', 'pdf-updated', performance.now(), performance.now() + 1);
    }
  }, [pdf]);

  useEffect(() => {
    if (typeof window !== 'undefined' && (window as any).setStateTracker) {
      (window as any).setStateTracker.trackStateChange('PDFViewerOpt3Combined', 'numPages-updated', performance.now(), performance.now() + 1);
    }
  }, [numPages]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        console.log(`[Opt3-Combined] PDF 로딩 시작: ${pdfSrc}`);
        ...
        console.log('[Opt3-Combined] 워커 초기화 완료, PDF 문서 로딩 시작');
        ...
        console.log(`[Opt3-Combined] PDF 문서 로딩 완료: ${loaded.numPages}페이지`);
        ...
        if (cancelled) return;
        console.error("[Opt3-Combined] PDF 로딩 실패:", e);
      ...
      (entries) => {
        entries.forEach(async (entry) => {
          ...
          if (entry.isIntersecting) {
            console.log(`[Opt3-Combined] Page ${pageNumber} is intersecting`);
            
            // 이미 렌더링되었는지 확인
            if (pageElement.rendered && pageElement.rendered()) {
              console.log(`[Opt3-Combined] Page ${pageNumber} already rendered`);
              return;
            }

            // 직접 렌더링
            if (pageElement.renderPage) {
              console.log(`[Opt3-Combined] Rendering page ${pageNumber} directly`);
              await pageElement.renderPage();
            } else {
              console.log(`[Opt3-Combined] renderPage function not found for page ${pageNumber}, element:`, pageElement);
              // 잠시 후 다시 시도
              setTimeout(() => {
                if (pageElement.renderPage) {
                  console.log(`[Opt3-Combined] Retry rendering page ${pageNumber}`);
                  pageElement.renderPage();
                } else {
                  console.log(`[Opt3-Combined] Still no renderPage function for page ${pageNumber}`);
                }
              }, 100);
            }
          }
      ...
      if (typeof window !== 'undefined' && (window as any).commitTracker) {
        ...
        setTimeout(() => {
          ...
          (window as any).commitTracker.trackCommit('dom-initialization', commitStartTime, commitEndTime, {
            totalPages: numPages,
            type: 'opt3-combined-all-pages-at-once'
          });
        }, 100);
      }
    }
  ...
  if (!pdf) return <div>PDF 로딩 중... (Opt3-Combined)</div>;
                ...
                if (typeof window !== 'undefined' && (window as any).commitTracker) {
                  ...
                  setTimeout(() => {
                    ...
                    (window as any).commitTracker.trackCommit('page-dom-added', commitStartTime, commitEndTime, {
                      pageNumber,
                      type: 'opt3-combined-initial-render'
                    });
                  }, 50);
                }
